		...
		- Siano $\phi_n \in \L2$ con $n \in \Z$.
		  ...
		  L'insieme di funzioni $\{\phi_n\}_{n \in \Z}$ si dice **base ortonormale** di $\L2$ se:
			...
			- logseq.order-list-type:: number
			  ...
			  \end{aligned}
			  $$
		- Dall'espressione di $f$ tramite la sua serie di Fourier si comprende intuitivamente che ogni coefficiente di Fourier $c_n$ indica "quanto" della funzione $\phi_n$ è presente nel segnale $f$. Nei seguenti casi particolari si identificano $\phi_n$ con le frequenze pure.
